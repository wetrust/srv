        ...
        $.get(serverURL + "dicom/getimages/" + $(this).val()).done(function(data) {
            ...
            if (data.DCM = true) {
                $.each(data.JPGFiles, function(i, item) {
                    $("#fotosDicom").append("<div class='col-3'><img class='rounded' alt='200x200' style='width: 250px; height: 250px;' src='" + serverURL + "data/" + RUTPACIENTE + "/" + item + "'><div class='form-check'><label class='form-check-label'><input type='checkbox' class='form-check-input' name='fotosElegidas'>Seleccionar</label></div></div>");
                });
